...
// 🔴 המקרה של עמי, 30/08/2026: בפריוריטי הלקוח כתוב "חלפ עבד אל כרים"
// (פ רגילה), והחיפוש "חלף" עם ף סופית החזיר אפס תוצאות.
test('🔴 אות סופית בשאילתה מוצאת אות רגילה במחסן', () => {
  assert.equal(matchesSearch('חלפ עבד אל כרים 053-4278092', 'חלף'), true);
});

test('אות רגילה בשאילתה מוצאת אות סופית במחסן, גם ברב-מילים', () => {
  assert.equal(matchesSearch('כהן אברהם תל אביב', 'אברהמ כהנ'), true);
  assert.equal(matchesSearch('יוסף לוי', 'יוספ'), true);
});
