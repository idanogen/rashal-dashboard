...
/**
 * 🔴🔴 עידן, 25/08/2026: חיפוש "שלומי קורן" בתיבה לא מצא את השיחה של
 * "קורן שלומי-שלמה", והלקוח הוצג במקום זה תחת "לקוחות בלי שיחה" עם
 * הצעה לשלוח תבנית, בזמן שחלון 24 השעות שלו פתוח והוא ממתין 56 דקות.
 * זהו מימוש שני של אותה תקלה שתוקנה באותו יום ב-`customer_search`.
 */
test('🔴 שם מלא נמצא גם בסדר הפוך', () => {
  const i = item({ id: 'a', name: 'קורן שלומי-שלמה' });
  assert.ok(matchesQuery(i, 'קורן שלומי'), 'הסדר המקורי');
  assert.ok(matchesQuery(i, 'שלומי קורן'), 'הסדר ההפוך');
});

test('🔴 נדרשות כל המילים ולא אחת מהן', () => {
  const i = item({ id: 'a', name: 'קורן שלומי-שלמה' });
  assert.ok(!matchesQuery(i, 'שלומי לוי'));
  assert.ok(!matchesQuery(i, 'כהן קורן'));
});

test('חיפוש מילה בודדת לא שינה התנהגות', () => {
  const i = item({ id: 'a', name: 'קורן שלומי-שלמה' });
  assert.ok(matchesQuery(i, 'קורן'));
  assert.ok(!matchesQuery(i, 'לוי'));
});

// 🔴 המקרה של עמי, 30/08/2026: בפריוריטי "חלף" כתוב "חלפ" (פ רגילה),
// וחיפוש עם האות הסופית החזיר אפס. אותו נרמול כמו search-match.ts.
test('אותיות סופיות מנורמלות בחיפוש התיבה', () => {
  const i = item({ id: 'a', name: 'חלפ עבד אל כרים', phone: '0534278092' });
  assert.ok(matchesQuery(i, 'חלף'), 'ף סופית לא מצאה פ רגילה');
  assert.ok(matchesQuery(i, 'כרים חלף'), 'רב-מילים עם אות סופית');
});
