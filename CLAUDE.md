# [לקוח] Rashal Dashboard — דשבורד הזמנות לר.שעל ציוד רפואי

## זיהוי פרויקט
- **פרויקט:** rashal-dashboard
- **נתיב:** `/Users/idanogen/Projects/rashal-dashboard`
- **סוג:** לקוח (ר.שעל ציוד רפואי)
...
### 26/08/2026: מגבלות heyy נמדדו, והערוץ מנוטר ⭐⭐⭐

**הבקשה (עידן):** "נכון לעכשיו המערכת בקושי עובדת, אבל מרגע שהיא תהיה
בפרודקשן אמיתי אני חייב לבדוק שאין הגבלות בתחום הזה."

**הכל נמדד חי מול החשבון, ולא נלקח מהתיעוד.** התיעוד המלא, כולל תוכנית
ההגירה: `docs/heyy-limits.md`.

🔴🔴 **הממצא המסוכן: heyy משביתה אוטומטית וובהוק שנכשל ברצף.** מאותו
רגע תשובות הלקוחות מפסיקות להגיע בלי שום סימן, וההפעלה מחדש ידנית
בממשק שלהם. השומר הקיים ניטר רק את משיכות פריוריטי.
⭐ **הפיזור שנמדד הוא מדד הבריאות הזול ביותר:** כל הודעה יוצאת מייצרת
~4.6 קריאות וובהוק (`sent` פעם אחת, `updated` בממוצע 3.59), ונכנסות
ויוצאות ביחס **1:1 מדויק**, כלומר אפס ניסיונות חוזרים והאנדפוינט שלנו
מעולם לא נכשל.
**הבדיקה החדשה** (`supabase/functions/rashal-watchdog/heyy-health.ts`,
טהורה ולכן נבדקת ביחידה) שואלת את heyy מה מצב הוובהוק: קיים · `active` ·
מצביע עלינו · **נושא את הסוד ב-`?k=`** · שלושת האירועים על הערוץ שלנו ·
הערוץ פעיל · דירוג האיכות לא ירד. 🔴 "לא הצלחתי לשאול" הוא מצב שלישי
ומתריע רק אחרי שתי בדיקות רצופות שנכשלו.
🔴 **אומת במוטציה חיה:** הפניה לכתובת שגויה החזירה `ALERT sent` ומייל
אמיתי, והחזרה החזירה `RECOVERED` ומייל שני.

🔴🔴 **ובמקביל, תקלה שקטה: לא היה שום טיפול ב-429.** הוא נשמר כ-`failed`
כאילו התוכן נדחה, ומנוע הסקרים סימן את השורה ככישלון ושחרר אותה רק
לבדיקה ידנית. כלומר **סקר שנחסם על מכסה נעלם לתמיד**, דווקא בשעה
העמוסה. עכשיו הוא מסומן `retryable` **וחוזר ל-`pending`**.

| מה | נמדד |
|---|---|
| מכסת heyy | **100 בקשות לדקה**, חלון קבוע, תוכנית Hobby |
| 🔴 בקשה שנכשלת | **נספרת במכסה** (חמישה 400 הורידו 99→94) |
| הדלי | **משותף** ל-`v3` ולנתיב הישן `api/v2.0` |
| מכסת מטא | 10,000 שיחות ל-24 שעות, איכות `high` |
| 🔴 אימות עסק במטא | **`isVerified: false`**, וזה מה שחוסם עלייה מעל התקרה |

🔴 **מדיניות הניסיון החוזר, ולמה היא לא סימטרית:** 429 בטוח כי הוא אומר
במפורש שהבקשה לא בוצעה, ולכן ממתינים עד 15 שניות ושולחים. **5xx על
שליחה לא נשלח שוב לעולם**, כי ייתכן שהוואטסאפ כבר יצא ללקוח וכפילות
גרועה מהודעה חסרה. 4xx אחר לא נשלח שוב כי הוא רק שורף מכסה.
⭐ וההמתנה קצרה בכוונה: חלון המכסה הוא דקה ופונקציית Vercel חיה 60
שניות, ולכן **התור הוא התיקון האמיתי וההמתנה היא קיצור דרך למקרה הקל**.

🔴🔴 **תאריך מת: `API v2` של heyy נסגר ב-1 בנובמבר 2026.** שליחת טקסט
חופשי, מנוע הסקרים והתזכורת היומית רצים היום על
`api.heyy.io/api/v2.0`. ב-v3 יש `POST /messages/send`, והתוכנית המלאה
עם החוזה המאומת נמצאת ב-`docs/heyy-limits.md`. חלון ביצוע: עד סוף ספטמבר.

**ובנוסף:** ווידג'ט המשוב הצף הוסר מהדשבורד לבקשת עידן (הסרת הסקריפט,
לא הסתרה ב-CSS).

⏸️ **פתוח וממתין לעידן:** 🔴 טרם נשלחה ולו הודעת תיאום אמיתית אחת ללקוח ·
🔴 `api/heyy-send` ופונקציית הסנכרון עונים **בלי טוקן** · אימות העסק
במטא · 13,246 איסופים "פתוחים" לפי הסטטוס.

### 25/08/2026 (לילה): ארבעה כשלים שקטים, כולם מדיווח של עידן ⭐⭐⭐⭐

🔴🔴 **א. מסך הסדרן הראה 0 הזמנות, והנתונים היו תקינים לגמרי.**
עידן: "אין סיכוי שאין הזמנות, והמסך עולה ממש לאט." במסד ישבו **735
הזמנות ממתינות לתיאום**. השורש: `dataWindowFilter()` היה
`created_at >= cutoff or updated_at >= cutoff`, והכוונה נכונה (רשומה
ישנה שעדיין מטפלים בה לא נעלמת). אבל **הייבוא ההיסטורי של אותו בוקר
כתב מחדש את כל ההזמנות**, ולכן `updated_at` של כולן הפך להיום.

| | לפני | אחרי |
|---|---|---|
| הזמנות שנטענות | **40,402** | 1,571 |
| קריאות שירות | 13,894 | 2,376 |
| סבבי רשת סדרתיים | 41 | 2 |

⭐ התיקון מחזיר את הכוונה: רשומה ישנה נשארת רק אם נגעו בה בחלון **וגם
היא אינה סגורה** (`ORDER_CLOSED` / `CALL_CLOSED` / `PICKUP_CLOSED`).
אומת שכל 735 ההזמנות וכל 459 הקריאות הפתוחות נשמרות אחת לאחת, **ושתחביר
`or=(a,and(b,c))` של PostgREST עובד מול השרת האמיתי עם הרשאה אמיתית**
(1,571 ו-2,376 בדיוק). 🔴 ערכים בעברית ב-`not.in` חייבים מרכאות כפולות.
[[row_touched_is_not_record_active]]

🔴🔴 **ב. לקוח עם שיחה פתוחה הוצג כ"בלי שיחה".** חיפוש "שלומי קורן"
בחלונית החזיר "אף לקוח לא מחכה", ומתחת את הלקוח תחת "לקוחות בלי שיחה"
עם כפתור "שלח תבנית" בתשלום, בזמן שהשיחה שלו פתוחה וממתינה 56 דקות.
שתי תקלות שהצטרפו: `matchesQuery` התאימה מחרוזת **רצופה** (מימוש שני
של אותה תקלה שתוקנה באותו יום ב-`customer_search`), ו"לקוחות בלי שיחה"
נגזר מ-`items` שהיא הרשימה **המסוננת**. השרת מחזיר עכשיו `phones` עם
כל השיחות לפני הסינון. [[priority_customer_name_has_no_order]]

🔴 **ג. "פתחתי כמה פעמים ועדיין יש 1".** הקוד עבד כפי שתוכנן: השורה
הראשונה נבחרת לבד, השרשור מצויר, והשורה עצמה מעולם לא נלחצה. הבחירה
האוטומטית אינה קריאה **בכוונה**, ולכן נוסף כפתור **"סמן כנקרא"** בכותרת
השרשור, שמוצג רק כשהשיחה באמת ממתינה. אומת שנקודת הקצה עצמה תקינה.
[[render_is_not_a_user_event]]

🔴 **ד. המפה עלתה על הכותרת.** Leaflet נותן לפקדים שלו `z-index: 1000`
משלו, מול כותרת `sticky z-50`. התיקון הוא **`isolate`** על כרטיס המפה,
ולא הורדת ה-z-index של הרכיבים הפנימיים.
[[leaflet_isolate_under_sticky_header]]

**ובנוסף:** תאריך קטן על כל הערה ב"מה הלקוחות כתבו" (עם שנה), ותשובת
הסקר על ההודעה הפכה ל**תגובה עגולה צמודה לתחתית הבועה**, כמו בוואטסאפ.

### 25/08/2026 (ערב): סנכרון לקוחות, ומה שאי אפשר לשאול את פריוריטי ⭐⭐⭐

**הבקשה:** עידן עדכן טלפון ללקוח קיים ושאל מתי זה מתעדכן.
🔴🔴 **התשובה: לעולם.** המשיכה השוטפת מסננת `CREATEDDATE ge <watermark>`,
שהוא תאריך **פתיחת** הלקוח ואינו זז בעריכה. חל על טלפון, כתובת ועיר.

**כל החלופות נבדקו, ואין דלתא זולה:**
- `STATUSDATE` זז על רשומות ישנות (516 מאז 01/07, ביניהן לקוחות מ-2023)
  אבל **לא** על עריכת טלפון. ⭐ **הבקרה החיובית היא שהצילה**: בלי לשאול
  "והלקוח שאני יודע שנערך, הוא שם?" הייתי מחליף שדה ומדווח שתיקנתי.
- `UNLOADTIME` הוא `Edm.String` ולא תאריך.
- ✅ `CHANGES_LOG_SUBFORM` קיים ומדויק (עידן הצביע עליו): `FIELD`,
  `OLDVALUE`, `NEWVALUE`, `USERLOGIN`, `UDATE`. 🔴 אבל `any()` עליו
  מוחזר **403 מדף HTML של שער ה-CDN**, והנתיב הישיר
  `CHANGES_LOG_SUBFORM/UDATE ge X` מחזיר 200 **ומתעלם מהתאריך** (בקרת
  תאריך עתידי החזירה את אותן 200 שורות). ואין ישות עצמאית ליומן.
- 🔴 וה-`UDATE` שם הוא **שעון ישראל עם תווית `Z`**.

**לכן: סריקה מלאה פעם בשבועיים**, `20260825_customers_full_scan_cron.sql`,
1 ו-15 לחודש, **מפוצלת לשתיים** כי ריצה אחת של 42,752 השורות ארכה 142
שניות מול תקרה של כ-150 לפונקציית Edge. אומת שהחצאים מכסים בדיוק את
אותן שורות (22,000 + 20,752). כ-2.1% מהמכסה לריצה.
⭐ הריצה הראשונה מצאה **3 רשומות שהשתנו מתוך 42,752**, כלומר 1,888
הלקוחות בלי טלפון הם בלי טלפון **גם בפריוריטי**.

**כלים חדשים ב-`rashal-sync`** (קריאה בלבד): `probe-datefield` ·
`probe-subform` · `probe-changes` · `probe-filters`, ושתי ישויות
backfill: `customers_all` ו-`customers_pick` (לפי מספרי לקוח).

⏸️ **פתוח:** 🔴 `rashal-sync` ו-`api/heyy-send` עונים **בלי שום טוקן**,
וה-`PRIORITY_SYNC_SECRET` שבקוד אינו נאכף · 13,246 איסופים "פתוחים"
לפי הסטטוס, בעיית נתונים שלא נבדקה.

### 25/08/2026 (ערב): תיאום הגעה שיוצא באמת, והתשובה חוזרת ליומן ⭐⭐⭐⭐

**הבקשה (עידן):** "בוא נייצר תבנית של פניה ללקוח עם תאום של מועד אספקה."

⭐ **הלולאה כבר הייתה בנויה, ומעולם לא יצאה ממנה הודעה.** הדיאלוג
`ScheduleCoordinationDialog` קיים, הוא כותב `whatsapp_sent` לעצירה,
ו-`parseCustomerReply` כבר יודע לקרוא את התשובה. אבל `heyy/templates.ts`
הצביע על `DEMO-schedule-coordination`, **מזהה מדומה שמעולם לא הוגש
למטא**, ולכן שבע השליחות שרשומות אצלנו כולן `is_demo = true`.

**התבנית `rashal_visit_coordination`** הוגשה ואושרה **בקטגוריית שירות**
תוך פחות מרבע שעה. ארבעה משתנים בשם, כולם ערכים מובנים
(`customer_name` · `purpose` · `day` · `hours`), **בלי שום חריץ לטקסט
חופשי**, ושני כפתורי מענה מהיר: "מתאים לי" · "לא מתאים".
🔴 `purpose` הוא רשימה סגורה של ארבעה ערכים ולא הקלדה, כי אותו דיאלוג
משרת אספקות, איסופים וביקורי טכנאי, והנוסח מוקפא אצל מטא.

**שני פערים שקטים שנסגרו יחד:**
1. 🔴🔴 הדיאלוג שלח דרך `api/heyy-send`, **ולנתיב הזה אין בדיקה מול
   רשימת המושתקים**. הועבר ל-`api/wa-send`.
2. 🔴🔴 `heyy-webhook` כתב את תשובת הלקוח ל-`orders` בלבד **ומעולם לא
   נגע ב-`calendar_stops`**; רק הסימולטור שבדפדפן עדכן את העצירה, ולכן
   בדמו הכל נראה סגור. `wa_apply_coordination_reply()` סוגרת את זה,
   ומתנה את ההתאמה בעצירה ששלחנו עליה שאלה, בחלון של 14 יום, ובתאריך
   שלא עבר. בלי התנאים האלה "כן" של לקוח היה מאשר עצירה אקראית, ויש
   כאן מאות עצירות פתוחות בתאריכי עבר.

**`src/lib/coordination-message.ts`** מחזיק את הנוסח, המשתנים וכיתובי
הכפתורים במקום אחד, בלי ייבוא, ולכן נבדק ביחידה. 🔴 הבדיקה החשובה
מריצה את **שני כיתובי הכפתורים דרך המפרש האמיתי בשני העותקים שלו**
(`api/_lib/extract.ts` ו-`src/lib/heyy/extract.ts`): כיתוב שהמפרש לא
מזהה פירושו עצירה שנתקעת ב"נשלח" לנצח, בלי שגיאה.

🔴 **ותקלת ניסוח שנתפסה בבדיקה ולא בעין:** התבנית אומרת "ביום
`{{day}}`" והפורמט החזיר "יום רביעי", כלומר **"ביום יום רביעי"** בהודעה
שיוצאת ללקוח.

**אימות:** 192 בדיקות (11 חדשות) · הפונקציה במסד נבדקה חי מול בקרה
חיובית ושלוש שליליות ושורות הבדיקה נמחקו (841 עצירות לפני ואחרי) ·
הדיאלוג צולם ברינדור אמיתי דרך `preview.html` · פרוס
(`1ecd77c`), ואומת שהחבילה בפרודקשן מכילה את הקוד החדש.

**ואחרי הפריסה, מדיווח של עידן:** "שלח תבנית" נוסף גם לשורות של
"לקוחות בלי שיחה" בתיבה (אותו `TemplateSendDialog`, בלי רכיב שני),
🔴 עם חיווי מפורש למי שאין לו טלפון (1,897 מתוך 42,757, כלומר 4.4%),
במקום שורה שנראית כמו כל השאר ופשוט אין בה כפתור.

🔴🔴 **וחיפוש שם מלא היה מחזיר אפס.** `customer_search` התאים את
השאילתה כמחרוזת **רצופה** אחת, ובפריוריטי אין סדר קבוע בין שם פרטי
למשפחה. נמדד: "אברהם לוי" החזיר **0** תוצאות (22 אחרי), "דוד כהן"
החזיר 4 (21 אחרי), ו"שלומי קורן" לא מצא את "קורן שלומי-שלמה". נוסף ענף
של **כל המילים בכל סדר**, בענף נפרד ולא כ-`or`, כדי שהאינדקס הטריגרמי
יישאר בשימוש: 6.9ms, והוא מסנן 52 מועמדים ל-2.
[[priority_customer_name_has_no_order]]

**וחיווי הסקר בתיבה** (עידן: "אנחנו לא חייבים להסתמך רק על נתוני
הווצאפ, נכון?"). נכון, ואצלנו יושב גם **הציון**: `src/lib/survey-badge.ts`
מחזיר אימוג'י, ציון וטון, ו-`api/conversation` מצרף לכל שורה בתיבה את
תשובת הסקר האחרונה של הלקוח.
🔴 **לא סמיילי אחיד:** מתוך 23 התשובות הראשונות 20 נתנו 5 ואחת נתנה 2,
וסימן זהה לכולם היה קובר בדיוק את זו שצריך לטפל בה. הסף 3 ומטה, כי מי
שנתן 4 מרוצה. [[color_on_everything_is_not_color]]
🔴 **ורשומות בדיקה מסוננות**, נתפס באימות החי: החיווי הראשון שהופיע על
המסך היה "😞 2" של רשומת בדיקה. 7 מתוך 23 התשובות היו בדיקות שלנו.
**המספרים האמיתיים של המנוע:** 37 יצאו, 16 נענו, **43% מענה**, ואפס
ציונים של 3 ומטה.

⏸️ **פתוח:** טרם נשלחה ולו הודעת תיאום אמיתית אחת ללקוח · תבנית שנייה
להודעה על מועד סגור (בלי שאלה) לא נבנתה · `heyy/templates.ts` נשאר
כקוד מת עם הערת אזהרה.

### 24/08/2026 (ערב): כרטיס לקוח מאוחד ⭐⭐⭐⭐

**הבקשה (עידן):** "אני רוצה מקום שירכז לי את ההיסטוריה של הפעילות עם
הלקוח... וכך כאשר לקוח מתקשר לנציג יש את כל המידע מול העיניים, כולל האם
יש משלוח פתוח ואם כן האם הוא שובץ לאספקה או לא."

🔴🔴 **המכשול היה הזיהוי, לא הפריסה.** נמדד לפני הבנייה: מספר לקוח קיים
ב-3,070 מתוך 7,477 ההזמנות (59% בלי), ב-2,971 מתוך 6,857 הקריאות,
**ובשורה אחת מתוך 840 השיבוצים ביומן**, ובאפס מתוך 29 הסקרים. מסך שמחבר
לפי מספר לקוח בלבד היה מציג כחצי מההיסטוריה **ונראה שלם**.

⭐ **לכן שלושה מפתחות בסדר יורד:** מספר לקוח ← טלפון מנורמל ← שם מדויק,
וכל רשומה נושאת את הוודאות שלה ומסומנת במסך כשהיא רכה.
🔴 **והכלל שמונע את רוב הטעויות: רשומה שיש עליה מספר לקוח אחר שייכת
למישהו אחר.** התאמה לפי טלפון או שם נבדקת רק על רשומות בלי מספר כלל.
⭐ הטלפון נמדד כמפתח: **4,774 מספרים מובילים ללקוח יחיד, 79 משותפים**
לשניים או שלושה, ואז המסך שואל במקום לנחש.

🔴 **"שובץ" נגזר מהעצירה ביומן ולא מהסטטוס.** נמדד: 16 הזמנות "תואמה
אספקה" בלי שום עצירה · 9 מבוטלות עם עצירה פעילה · 25 קריאות שנסגרו עם
עצירה פתוחה. הכרטיס מציג את מה שביומן ואומר את הפער במפורש.
**50 המקרים אינם באג של המסך אלא עבודה שממתינה.**

🔴 **ורשומה מאורכבת אינה רשומה שנעלמה.** היא בציר הפעילות, ואינה נספרת
כפתוחה אלא אם יש לה עצירה פעילה (אז נהג עדיין אמור לנסוע לשם). נמצא חי:
לקוח 013272224 מחזיק הזמנה מאורכבת עם עצירה מתוכננת ל-14/06.

**מה נבנה:**
| רכיב | תפקיד |
|---|---|
| `customer_search()` · `customer_card()` | כל הכרטיס בקריאה אחת מהמסד, 184ms. RPC ולא נקודת קצה חדשה, בגלל תקרת 12 הפונקציות של Vercel |
| `is_office_staff()` | ההרשאה נאכפת במסד, לא במסך. נהג אינו נכלל |
| `src/lib/customer-answer.ts` | המשפט שהנציג אומר ללקוח. בלי ייבוא, ולכן נבדק ביחידה |
| `components/customer/CustomerCard.tsx` | הכרטיס עצמו |
| `CustomerCardSheet` · `CustomerCardButton` | אותו כרטיס כמגירה, מכל רשימה |
| `pages/CustomerPage.tsx` (`/customer`) | מסך החיפוש |
| `preview.html` + `src/preview/` | רינדור הרכיב האמיתי בלי מסך התחברות, לצילום |

**שלוש נקודות כניסה, רכיב אחד:** מסך `/customer` · מגירה ממסך הסדרן
ומתיבת השיחות · ורצועת "פתוח אצל הלקוח" **בתוך החלונית שבפריוריטי**
(התוסף 0.19.0, נשען על אותה פונקציה דרך `api/priority-context`).

**אימות:** 155 בדיקות (10 חדשות על המשפט הנאמר) · אומת חי על לקוח אמיתי
(אסרף דוד): הזמנה אחת לפי מספר ואחת לפי טלפון, והפער בין הסטטוס ליומן
הוצג · הכרטיס והמגירה נראו בעיניים בדפדפן.

🔴 **תוקן בעקבות הצפייה החיה:** "יש משלוח פתוח, עדיין לא שובצו" (יחיד
ורבים), והמגירה נדחסה לשתי עמודות כי נקודות השבירה של Tailwind נמדדות
מול חלון הדפדפן ולא מול המיכל.

⏸️ **פתוח:** אנשי הקשר של הלקוח (`CUSTPERSONNEL_SUBFORM`) **אינם
מסונכרנים אצלנו**, ולכן אינם בכרטיס · אין יתרת חוב (לא מסונכרנת, וגם
דורשת החלטה מי רשאי לראות) · הכרטיס אינו מאחד לקוחות כפולים, רק חושף
אותם.

### 24/08/2026: תיבת השיחות, שלושה תיקונים מדיווחים של עידן ⭐⭐

**א. תמונה מוצגת, וכפתור תבנית מפסיק להתחזות לקובץ**

heyy מכניסה גם כפתורי תבנית לאותו מערך `attachments`. נמדד: **18 מתוך
27 המצורפים במסד היו כפתורים**, ולכן כל הודעת סקר הציגה "📎 קובץ מצורף"
שלחיצה עליו נכשלת. במקביל, תמונה אמיתית שלקוח שלח הוצגה כשורת אטב
במקום להיראות.

- **`api/_lib/attachments.ts`** (בלי ייבוא, ולכן נבדק ביחידה):
  `describeAttachments` מחזירה `{index, name, kind, ready, sizeBytes}`.
  ההכרעה נעשית **פעם אחת בשרת**, ושני הצדדים מציירים ממנה: התיבה
  שבדשבורד והחלונית שבפריוריטי.
- 🔴 **המטען הגולמי הפסיק לנסוע לדפדפן.** בתוכו ישבו `file.url`, כתובת
  S3 שחיה 24 שעות ואינה דורשת הזדהות, והנתיב הפנימי שלנו בדלי.
- 🔴 **האינדקס נשאר של המערך המקורי.** בקשת המדיה נשלחת לפי מיקום,
  ומספור מחדש אחרי סינון היה פותח קובץ אחר.

**ב. שיחה שנקראה יורדת מ"ממתינים", גם בלי לענות**

עידן: "הודעה שכבר נקראה זה לא תמיד מסמן אותה כאחת כזאת". במסד היה מושג
אחד, `unanswered_since`, והוא נמחק **רק כששולחים תשובה**. לכן "תודה" של
לקוח נשאר תלוי לנצח, ושיחה שנקראה המשיכה לצעוק "מחכה 27 דקות".

- `wa_conversations.read_at` + `read_by`. **לצד `unanswered_since` ולא
  במקומה:** הלקוח עדיין ממתין לתשובה, וזו עובדה שלא נמחקת.
- `isWaiting()` ב-`_lib/inbox.ts` הוא המקום היחיד שמחזיק את הכלל, וגם
  הרשימה, גם המונה, וגם הנקודה על הבועה נגזרים ממנו.
- 🔴 **ההשוואה מול `last_inbound_at` ולא דגל בוליאני**, ולכן לקוח שכותב
  שוב חוזר לרשימה מעצמו.
- 🔴 **הסימון נרשם רק מלחיצה של אדם.** בחירה אוטומטית של השורה הראשונה,
  טעינת שרשור בפריוריטי, ורענון שקט כל 20 שניות אינם קריאה.
  הסימון רוכב על `GET /api/conversation?markRead=1` בגלל תקרת 12
  הפונקציות.

**ג. הודעה עם קובץ ובלי טקסט אינה כישלון** (`heyy-webhook`).

**אימות:** 139 בדיקות, ובבדיקת מוטציה שלוש חלופות לכלל ההמתנה מפילות
בדיוק את הבדיקות שלהן · אומת חי: לחיצה אחת הורידה את הממתינים מ-1 ל-0,
ובמסד נרשמה **שורה אחת בלבד** מתוך 31 שיחות, כלומר הבחירה האוטומטית
והרענון השקט לא סימנו כלום · התמונה נראתה בדפדפן.

### 23/08/2026 (לילה): מבוי סתום בדיאלוג הכפילויות, וגרירה בין טכנאים ⭐⭐⭐

**שתי הערות של עמי שהן בעיה אחת.**

#### א. הדיאלוג היה מבוי סתום

`DuplicateScheduleWarningDialog` הציג שני כפתורים בלבד: "הבנתי" ו"דלג על
הכפילויות". תיבת ההסבר שלו שלחה את העובד **לנווט ליום אחר ביומן, למחוק
שם שורה, ולחזור**. אין "שבץ בכל זאת", אין "העבר את הקיים".

🔴 **ולמה זה קרה כמעט תמיד:** `findActiveDuplicateStops` מסנן
`status in ('planned','in_progress')` **בלי שום חסם תאריך**. נמדד:
**295 מתוך 296 העצירות ה"פתוחות" יושבות בתאריך שכבר עבר**, הוותיקה
מ-22/04. עצירה מאפריל חוסמת שיבוץ באוגוסט.

- נוסף `onReschedule`: **"העבר את השיבוץ הקיים"** מזיז את העצירה הקיימת
  ליום ולעובד שנבחרו במקום ליצור שנייה. ההגנה במסד לא נגעה, כי אחרי
  ההעברה עדיין יש בדיוק שיבוץ פעיל אחד.
- "הבנתי" הפך ל"ביטול", וההעברה היא הפעולה הראשית.
- ⭐ ללקוח יש לכל היותר **עצירה פעילה אחת** (אינדקס ייחודי חלקי), ולכן
  `existing[0]` הוא כל מה שיש להעביר. ההעברות רצות **ברצף ולא במקביל**:
  המספר הסידורי נגזר מהעצירה האחרונה של אותו עובד באותו יום.

#### ב. אי אפשר היה להחליף טכנאי

```js
if (srcDriver !== overDriver) return;                  // stop → stop
if (!ref || ref.deliveryDate === newDate) return;      // startReschedule
```

שתי השורות האלה חסמו את שני המסלולים, כלומר **לא הייתה שום דרך במערכת
להחליף טכנאי בלי להזיז גם את התאריך**, והגרירה לא עשתה כלום ולא אמרה
כלום. עכשיו שחרור על טכנאי אחר מעביר אליו, **בלי דיאלוג**, כי היעד ידוע
מהשחרור עצמו.

#### 🔴 ג. התראת שווא שהתגלתה בדרך

`rescheduleStop` סימן `coordination_needs_cancel` וכתב הודעת מערכת
"יש לבטל את התיאום" על **כל** העברה, גם כשרק הטכנאי התחלף. הלקוח קיבל
חלון זמן, לא שם. הדגל וההודעה מותנים מעכשיו בכך שהתאריך באמת זז.

**אימות:** ההעברה נבדקה חי על שארית אמיתית מהמסד בגלגול לאחור · הדיאלוג
צולם בדפדפן · 62 בדיקות · בילד ו-eslint ללא רגרסיה.

⚠️ **בדיקה שנפלה ולימדה:** הנחתי שאפשר לספור שיבוצים פעילים לפי שם
הלקוח. יש במסד שני לקוחות פעילים עם אותו שם ופרטים שונים. **מפתח הזהות
הוא שם+טלפון+כתובת+עיר**, כמו האינדקס.

⏸️ **פתוח: 295 השאריות עצמן.** הופק דוח לעיון
(`~/Idan-HQ/meetings/rashal/2026-08-23-עצירות-פתוחות-בעבר.html`), ולא
נגענו בנתונים. 🔴 והפילוח שם מסביר הכול: **לישראל, אולג ומוחמד אין
משתמש במערכת**, כלומר אין להם דרך לסגור עצירה מהשטח. ורודי, שיש לו
אפליקציה, משאיר 68% פתוחות מול 15% אצל אבי ודוד.

### 23/08/2026 (ערב): הצעות השלמה, וסיבת אי-הביצוע בעמודה משלה ⭐⭐⭐

**הבקשה (עמי):** "כשהקמתי לקוח באופן יזום המערכת זכרה ערים, סוג כיסא וכל
דבר שרשמתי, ומשום מה זה נעלם."

🔴 **מה שנעלם לא היה פיצ'ר שלנו.** נבדקה כל היסטוריית הקוד: אין ולא היה
`datalist`, אין רכיב השלמה, ואין `<form>` עם שדות בעלי `name` בשום מסך
מלבד ההתחברות. מה שהוא זכר הוא **זיכרון הטפסים של הדפדפן**, שדורש טופס
ושליחה אמיתית ולכן אינו קיים בדיאלוגים של React. הוא גם פר-מכשיר
ופר-פרופיל, ולכן "נעלם" בלי שאיש נגע בו.

**לכן זה נבנה, לא הוחזר**, ומהנתונים שלנו:

| שדה | מקור ההצעות | כמות |
|---|---|---|
| עיר | `orders` + `service_calls` + `calendar_stops` | 611 |
| הערות ("סוג כיסא") | `service_calls.device_name` + תיאורי משימות | 183 + 34 |
| שם לקוח | משימות שנפתחו ביוזמה | 152 |
| כתובת | אותן משימות | 140 |

- `field_suggestions()` RPC, נטען פעם אחת לסשן.
- `src/lib/suggestion-filter.ts` (בלי שום ייבוא, ולכן נבדק ביחידה) +
  `src/components/SuggestInput.tsx`.
- ⭐ **התאמה מתחילת המילה קודמת להתאמה באמצע**: מי שמקליד "רמ" מחפש את
  רמת גן ולא את כרמיאל.
- 🔴 הרשימה נפתחת רק אחרי מיקוד או הקלדה, אף פעם לא מרינדור.
- 🔴 **`if not <null>` אינו מרים חריגה**, ולכן משתמש מחובר בלי שורת
  פרופיל היה מקבל את הרשימות. `is not true`. אומת חי בשלוש זהויות:
  מנהל צוות עובר · נהג נחסם · משתמש בלי פרופיל נחסם.

#### 🔴 ותוך כדי: איבוד נתונים שקט ב-`notes`

`resolveStop` כתב את **סיבת אי-הביצוע** לתוך `calendar_stops.notes`,
אותה עמודה שבה יושב **תיאור המשימה** שנרשם בהקמה, ודרס אותו.
**27 משימות כבר איבדו את התיאור המקורי שלהן.**

- נוספה `resolution_note`. `notes` לא נדרס יותר לעולם.
- `buildReturnedMap` קורא `resolutionNote ?? notes`, כי עצירות היסטוריות
  מחזיקות את הסיבה ב-`notes`.
- בלי ההפרדה גם ההצעות היו מזוהמות: מי שפותח משימה היה מקבל
  "הלקוח לא היה בבית" כהצעה לתיאור.

**אימות:** 62 בדיקות · הרשימה נראתה בעיניים בדפדפן (הקלדה "רמ" מחזירה
רמת גן, רמלה, ואז כרמיאל, ולחיצה ממלאת) · הבילד ו-eslint ללא רגרסיה.

### 23/08/2026: צוות השטח יצא מהקוד, ותפקיד "מנהל צוות" ⭐⭐⭐⭐

**השאלה (עמי):** "האם אני יכול להוסיף טכנאי או נהג חדש, או שזה רק דרככם?"
התשובה הייתה "רק דרכנו", בשתי שכבות נפרדות. שתיהן נסגרו.

#### א. `assignees`, ומסך `/admin/team`

שבעת שמות הצוות ישבו כטיפוס `driver_name` במסד ובשלושה קבצים
(`types/route.ts` · `types/delivery.ts` · `api/admin-users.ts`). קליטת
עובד בשטח הייתה שינוי קוד ופריסה.

- **טבלה חדשה `public.assignees`:** `name` (מפתח ראשי) · `kind`
  (driver/technician/both) · `phone` · `color` · `active` · `sort_order`.
- `AssigneeName` הפך ל-`string`. `DRIVERS` / `TECHNICIANS` / `ASSIGNEES` /
  `TECHNICIAN_ONLY` / `ASSIGNEE_PHONES` / `DRIVER_CONFIG` **נמחקו**,
  והמקור היחיד הוא `useAssignees()`.
- 🔴 **הצבע נשמר כמפתח מהפלטה ולא כמחלקת Tailwind.** Tailwind גוזר
  מחלקות בזמן בנייה; מחרוזת שמגיעה מהמסד לא תיווצר לעולם. הפלטה
  (13 צבעים) ב-`types/assignee.ts`.
- 🔴 **`assigneeStyle` נשען על מטמון מודול** ש-`useAssignees` מזין
  (`primeAssigneeStyles`), כי היא נקראת מתאי יומן וממרקרים על המפה שאין
  להם גישה ל-hook. נפילה לאפור ניטרלי, והשם תמיד מוצג.
- **אין מחיקה במסך, בכוונה.** השם הוא המפתח שכל העצירות ההיסטוריות
  מחזיקות. מי שעזב מסומן לא פעיל. מפתח זר עם `on update cascade` מטפל
  בשינוי שם.

🔴 **ההמרה מ-enum לטקסט נחסמת על ידי RLS.** שלוש-עשרה מדיניויות משוות מול
`current_user_driver()`, וכל אחת חוסמת `alter column ... type`. הן נשמרו
מ-`pg_policies` והוחזרו מהטקסט השמור בלולאה, **ולא נכתבו מחדש ביד**:
שכתוב ידני של מדיניות אבטחה הוא המקום שבו טעות הקלדה פותחת נתונים בשקט.
**אימות:** `md5` של כל 73 המדיניויות זהה לפני ואחרי, 804 העצירות שלמות,
הטיפוס `driver_name` הוסר.

#### ב. תפקיד `team_manager`

עמי היה `dispatcher`, ומסך המשתמשים היה פתוח ל-`admin` בלבד.
`team_manager` = סדרן **ועוד**: מנהל משתמשים ואת צוות השטח.

🔴 **שלושה איסורים שמפרידים אותו ממנהל מערכת:**
1. **אין מחיקת משתמש.** השבתה הפיכה, מחיקה לא.
2. **אסור לגעת במשתמש שהוא מנהל מערכת.** מי שמאפס לו סיסמה הוא מנהל
   מערכת בפועל.
3. **אסור להעניק "מנהל מערכת".** אותה הסלמה, דלת אחרת.

ההכרעה ב-**`api/_lib/user-admin-policy.ts`** (בלי ייבוא יחסי, ולכן נבדק
ביחידה). **התפקיד של היעד נקרא מהמסד ולא מהבקשה**, אחרת ההגנה נשענת על
הצהרה של הלקוח. `ProtectedAdminRoute` קיבל `allow`; `/admin/wa-templates`
נשאר `admin` בלבד.

**אימות:** 7 בדיקות + בדיקת מוטציה (ביטול כל הגנה מפיל בדיוק את הבדיקה
שלה) · הבילד נקי · מסך הצוות נראה בעיניים ברינדור אמיתי של הרכיב מול
ה-CSS המהודר, ותוקן בעקבותיו (13 נקודות צבע בשורה הציפו את הטבלה).

⏸️ **פתוח:** מי שמוגדר "צפייה בלבד" עדיין רואה את מסך הסדרן ואת כפתורי
הפעולה, והם נכשלים ב-RLS. לא דליפה, אבל מסך שמבטיח פעולה שלא תקרה.

### 23/08/2026: הסיבה שהנהג רשם מגיעה למנהל ⭐⭐

**הדיווח (עמי, 19/08):** נהג סימן קריאה כ"לא בוצע" ורשם הערה, והמנהל
רואה כרטיס אדום בלי מילה ממנה.

🔴 **השורש, ומשפחת הכשלים שהוא שייך אליה:** `NotCompletedReasonDialog`
**חוסם** את הנהג מלסמן בלי לכתוב סיבה, הסיבה נשמרת נכון
ב-`calendar_stops.notes`, והיא הוצגה **רק ב-`DriverDashboardPage`**,
כלומר בפני היחיד שכבר יודע מה הוא כתב. כל מסכי המנהל קיבלו
`returnedIds: Set<string>`, מבנה שאין בו מקום פיזי לטקסט. שדה חובה
שאין לו צרכן.

**המקרה בפועל:** גובנר חנה (`SC2602711`), נהג דוד, 19/08 16:20,
"טופל אתמול על ידי ישראל". המייל של עמי נשלח ב-17:03.

**מה נבנה:**
- **`src/lib/returned-from-route.ts`**, `buildReturnedMap(stops, source)`
  מחזיר `Map<entityId, {note, driver, deliveryDate, stopId}>`.
  🔴 **הניסיון האחרון מנצח** (תאריך הקו, ובאותו יום `completed_at`),
  כי ללקוח שנוסו אצלו פעמיים הסיבה הרלוונטית היא של הפעם האחרונה.
  `returnedIdSet(map)` למי שרק מסנן.
- **`src/components/ReturnedNote.tsx`**, שורת הסיבה. יושבת בכל מקום
  שיש בו חיווי "חזר מהקו": שתי רצועות הדשבורד ו-`DispatchCard`.
  התג עצמו נושא את הסיבה גם ב-`title`.
- **שש הגזירות הכפולות** של `returnedIds` (DashboardPage ×2,
  DispatchPage ×3, DeliveriesPage, ServiceCallsPage, PickupsPage)
  אוחדו לבנאי אחד.

**אימות:** בילד נקי · `test/returned-from-route.test.mjs` (7 בדיקות),
ובבדיקת מוטציה החלפת "האחרון מנצח" ב"הראשון מנצח" מפילה שתיים · נבדק
ויזואלית מול ה-CSS המהודר (התיבה, הגלישה של הערה רב-שורתית, ומצב
"לא נרשמה סיבה"). eslint: 58 בעיות לפני ואחרי, אף אחת בקבצים החדשים.

⏸️ **פתוח:** הסיבה עדיין לא נכתבת חזרה לפריוריטי. מי שמסתכל בקריאה
עצמה (`priority_status='שובצה'`) לא רואה שהנהג לא הגיע ולמה.

### 12/08/2026: ארבע רשימות הממתינים על רכיב אחד ⭐⭐⭐

**הבעיה:** במסך הסדרן ארבע רשימות ממתינים, וכל אחת נראתה והתנהגה אחרת.
הזמנות וקריאות שירות נבנו בפברואר-יוני וצברו פיצ'רים גל אחרי גל (774 ו-730
שורות). איסופים ולקוחות חדשים נכתבו מאוחר יותר מאפס, לא כשכפול של הקיים
(274 ו-333 שורות). זו הצטברות היסטורית, לא החלטת עיצוב.

**מה נבנה:** `src/components/dispatch/UnscheduledPanel.tsx` מחזיק את הפאנל
ואת הכרטיס. כל סוג ממפה את עצמו ל-`DispatchItemVM` וזהו. ארבעת הרכיבים
הפכו למתאמים דקים (109 עד 151 שורות), **ה-props שלהם לא השתנו**, ולכן
`DispatchPage` לא נגע בכלל.

| קובץ | לפני | אחרי |
|---|---|---|
| `dispatch/UnscheduledPanel.tsx` | לא היה | 737 (חדש) |
| `deliveries/UnscheduledOrders.tsx` | 774 | 131 |
| `service-calls/UnscheduledServiceCalls.tsx` | 730 | 130 |
| `customers/UnscheduledCustomers.tsx` | 333 | 151 |
| `pickups/UnscheduledPickups.tsx` | 274 | 109 |

**`DispatchItemVM` - החוזה שכל סוג ממלא:**
`id` · `dragId` · `dragData` · `zoneId` · `customerName` · `customerNumber` ·
`phone` · `addressLine` · `created` · `dupCount` · `searchText` (מוכן
ב-lowercase) · `meta` (שורות ייחודיות לסוג) · `nameBadge` · `footerBadges` ·
`renderDetail` (דיאלוג שמנוהל בכרטיס) או `onShowDetails` (דיאלוג שמנוהל בדף) ·
`history`.

🔴 **`dragId` ו-`dragData` חייבים להישאר בדיוק כפי שהיו** (`order-<id>` ·
`servicecall-<id>` · `pickup.id` · `customer.customerNumber`, ו-`type` בתוך
ה-data). ה-handlers ב-`DispatchPage` מזהים לפיהם ואין ביניהם חוזה מטיפוס.

**מה קיבלו איסופים ולקוחות חדשים:** מבנה פאנל אחיד (כותרת, ספירה, סמן-הכל,
חיפוש, בורר תצוגה וכיווץ בשורה דביקה אחת) · קיבוץ לפי אזור · רצועת "חזרו
מהקו" · בחירה וגרירה מכל הכרטיס במקום צ'קבוקס וידית · באדג' ותק · טלפון
לחיץ · שורת כתובת · הסרה זמנית והחזרה · כפתור היסטוריית לקוח.

**נשמר בכוונה:** פס הצד הצבעוני לפי סוג (כחול · כתום · טורקיז · סגול),
תיבת ההסבר הסגולה של לקוחות חדשים, המתג "רק ללא הזמנה", דיאלוג פרטי האיסוף,
ו**מפתחות ה-localStorage של הכיווצים** (`collapse:{orders|calls|pickups|customers}-*`),
כדי שההעדפות של הסדרן לא יתאפסו.

**תוקן תוך כדי:** כשהמתג "רק ללא הזמנה" סינן לאפס לקוחות, הרשימה נעלמה
ואיתה המתג, ולא הייתה דרך לצאת ממצב הריק. מצב הריק מציג עכשיו את
`toolbarExtra`.

**סטטוס אימות:** בילד ו-eslint נקיים, נפרס לפרודקשן (`cbb604e`).
**לא בוצע אימות ויזואלי** - הגרירה מכל הכרטיס באיסופים ובלקוחות היא
האזור עם הסיכון הגבוה ביותר וטרם נבדקה בעיניים.

#### המשך: חיפוש וסינון אחד לכל סוגי המסמכים

לכל רשימה היו תיבת חיפוש וסינון אזור משלה. בטאב "הכל" זה אמר ארבעה
עותקים של אותו סינון, וחיפוש שם לקוח דרש להקליד אותו ארבע פעמים.

- **`components/dispatch/DispatchFilterBar.tsx`** חדש: חיפוש אחד וסינון
  אזור אחד, מוצג פעם אחת ב-`DispatchPage` מעל הרשימות. מציג `X מתוך Y`
  וכפתור "נקה סינון" כשמשהו פעיל.
- **`UnscheduledPanel`** קיבל `search` ו-`selectedZones` כ-props אופציונליים.
  כשהם מועברים הפאנל **לא מצייר** את החיפוש ואת `ZoneFilter` בעצמו, ורק
  מסנן לפיהם. בלעדיהם הוא חוזר ל-state מקומי (הדפים המתים עדיין עובדים).
- **`components/dispatch/items.tsx`** חדש: ארבעת בנאי ה-VM (`buildOrderItems` ·
  `buildServiceCallItems` · `buildPickupItems` · `buildCustomerItems`), משותפים
  לרכיבים העוטפים ול-`DispatchPage` (שבונה אותם כדי לספור אזורים והתאמות).
- **החיפוש רץ על כל השדות**, לא רק שם/מספר/טלפון: כתובת, עיר, קופ"ח,
  סוכן, מי פתח, סטטוס (שלנו ושל פריוריטי), פריטי ההזמנה (מק"ט · תיאור ·
  סריאלי), מכשיר ותקלה בקריאות, ומספר מסמך · תעודה · מחסן · שורות האיסוף.
- מתג "רק ללא הזמנה" של הלקוחות הורם ל-`DispatchPage` (`customersOnlyBare`),
  אחרת ספירת האזורים בסרגל לא תואמת את מה שהרשימה מציגה.

### 09/08/2026 (ערב) — ריצת השוואה מול פריוריטי ⭐⭐⭐⭐

**הבעיה שנחשפה:** כל ארבע המשיכות הן הפרשיות בלבד (`CREATEDDATE/STATUSDATE/UDATE ge <watermark>`).
רשומה נכנסת רק ברגע שהשדה חוצה את הווטרמרק. מה שנוצר לפני תחילת הסנכרון, או
שהווטרמרק דילג עליו, **לא ייכנס לעולם** — אין שום ריצת השוואה. והוואצ'דוג לא
תופס את זה כי הוא סופר ריצות מוצלחות, לא תוכן.

**מה נבנה** (`rashal-sync`, קריאה בלבד, לא מתקן כלום):
- `probe-fields` — מדפיס את שמות השדות פר ישות. **לא מנחשים שמות שדות** (לקח #1 של רוני:
  `$select` עם שדה לא קיים מחזיר 400). גילוי: `CUSTOMERS` נושא גם `STATUSDATE`, לא רק `CREATEDDATE`.
- `reconcile` — `{"job":"reconcile","days":30,"entity":"orders"}`. מושך מצב מלא של חלון
  **בלי ווטרמרק**, משווה מפתחות מול הטבלה שלנו, ומחזיר `missing_in_us` + דגימה עם שם/טלפון/עיר.
- `reconcile-daily` — pg_cron job 8, `0 6 * * 0-4` (09:00 שעון ישראל). מתריע במייל (Resend,
  אותו ערוץ של הוואצ'דוג) **רק על חוסר אמיתי**: רשומה שנפתחה לפני היום. רשומה מהיום היא
  פיגור רגיל בין ריצות ולא חריגה, אחרת ההתראה נהיית רעש. היסטוריה ב-`reconcile_runs`.

**התוצאות של ההרצה הראשונה:**

| חלון | customers | orders | service_calls | pickups | חסר אמיתי |
|---|---|---|---|---|---|
| 30 יום | 288 | 392 | 454 | 166 | **0** |
| 180 יום | 1,423 | 2,198 | 2,180 | 1,241 | **1,764** |

🔴 **כל החסרים מתחילים ב-10/02/2026 ורצופים לפי מספר** (`SO2600650+`, `SC2600503+`,
`RT2600329+`) — זו נקודת ההתחלה של הסנכרון. החלטה פתוחה: לייבא או לא.

**לקח מתודי (חשוב מהקוד):** בניתי היפותזה שלמה על "חור במשיכה" מתוך **מספרי טלפון
שפוענחו מצילום מסך מטושטש בוואטסאפ**. ריצת ההשוואה הוכיחה את ההפך — הסנכרון שלם.
הפערים היו בקריאה, לא בנתונים. הצלבה מול צילום היא לא הצלבה. גיליון שני וברור
(מוחמד, 09/08, 16 שורות) נמצא **16/16 אצלנו וכולן "ממתין לתאום"**, כלומר תיקון
הסטטוס של הבוקר עובד על עבודה אמיתית.

**נשאר פתוח:** היומן ריק — 16 האספקות ברשימת הממתינים אך אף אחת לא משובצת;
הנהגים נוסעים לפי אקסל. ומסך הניקוי החד-פעמי לסדרן (~790 פעילות + 743 בהיסטוריה).

### 09/08/2026 — לקוח חדש כמקור עבודה + פריוריטי כבר לא סוגר הזמנות ⭐⭐⭐⭐⭐

**א. הבאג: הזמנות נסגרו לפני שסופקו**
המיפוי מ-05/08 סגר הזמנה כש-`ORDSTATUSDES` הוא `בוצעה` או `שולמה`. ברשעל
`בוצעה` נקבע בזמן החיוב, לעיתים ימים לפני הנסיעה. התוצאה: **73% מההזמנות
החדשות נולדו סגורות** (76 מתוך ~104 שנפתחו מ-01/08), והסדרן לא ראה אותן.
התגלה מגיליון האספקות של עמי ל-10/08: מתוך 13 שורות, המערכת הציגה אחת.

הכשל היה חצי-הגנה, לא ניחוש. ההערה ב-`rashal-sync/index.ts:231` כבר אמרה
ש-"בוצעה" נקבע לחיוב לפני הנסיעה, אבל ההגנה הוחלה על `calendar_stops` בלבד.
הזמנה שטרם שובצה לא היה מה להגן עליה.

- `ORDER_TERMINAL` (ב-`api/priority-sync.ts` וב-`supabase/functions/rashal-sync/index.ts`)
  מכיל מעכשיו **רק `מבוטלת`**. `CALL_TERMINAL` לא השתנה.
- **החזרת נתונים:** 969 הזמנות הוחזרו לסטטוס האמיתי שלהן; 743 שנפתחו לפני
  01/07/2026 סומנו `archived_reason='historic-cutoff-20260809'`.
  גיבוי מלא: `cleanup_snapshot_20260809`. הרשימה הפעילה 562 → 790.
- 🔴 **לקח:** קו חותך לפי גיל בלע 2 מ-6 האספקות של עמי (מאפריל ומיוני) והוחזרו
  ידנית. **גיל הוא לא פילטר טוב יותר מסטטוס**, רק פחות גרוע. בפריוריטי הזמנה
  יכולה לשבת חודשים ועדיין להמתין לאספקה.
- 🔴 **לקח על ניטור:** ה-watchdog סופר ריצות מוצלחות ולכן היה ירוק לאורך כל
  התקופה. ניטור שלא בודק תוכן לא יתפוס כשל כזה.
- דוח + SQL: `~/Idan-HQ/meetings/rashal/2026-08-09-{דוח-יבש-סטטוס-בוצעה.md,החזרת-סטטוס-בוצעה.sql}`

**ב. הפער האמיתי: אספקה נשענת על פתיחת לקוח, לא על הזמנה**
עידן, 09/08: ברשעל אספקה נשענת **ברוב המקרים על פתיחת לקוח חדש**. הזמנה לא
תמיד נפתחת, ואם נפתחת היא עלולה להיסגר מיד. `priority_customers` (1,176 שורות,
מסונכרנת) שימשה עד כה כ-cache להעשרת כתובות בלבד ולא הופיעה באף מסך.
**החודש 50 מתוך 81 הלקוחות החדשים היו בלי הזמנה** — אספקות בלתי נראות.

- **סוג עצירה שישי `customer`** + עמודה `customer_number` ב-`calendar_stops`
  (מיגרציה `20260809_new_customers_as_work_source.sql`). כמו `task` אין לו FK
  לישות, אבל מספר הלקוח חובה. `setSourceState` הוא no-op עבורו.
- **טאב "לקוחות חדשים"** ב-`/dispatch?tab=customers` — חיפוש, סינון אזורים,
  בחירה מרובה, גרירה ליומן ותזמון קבוצתי, בדיוק כמו שאר הטאבים. ברירת המחדל
  מציגה רק לקוחות ללא הזמנה; מתג מציג את כל החדשים.
- קבצים חדשים: `types/customer.ts`, `lib/customers.ts`, `hooks/useNewCustomers.ts`,
  `components/customers/UnscheduledCustomers.tsx`.
- 🔴 **הוספת סוג עצירה נוגעת ב-7 מפות סוגים.** הקומפיילר תפס את כולן, כולל
  **אפליקציית הנהג** (`DriverDashboardPage`) שהייתה מציגה כרטיס שבור:
  DeliveryCalendar · DayMapDialog · RouteMap · ScheduledOverview · DriverDashboardPage ·
  `lib/activity.ts` · `types/delivery.ts`. אם מוסיפים סוג נוסף בעתיד — לחפש
  `SOURCE_META|SOURCE_CONFIG|sourceConfig|sourceIconInPopup`.

### 05/08/2026 — ביצועים + ניקוי זבל: סטטוס מפריוריטי כמקור אמת לסגירה ⭐⭐⭐⭐⭐

**א. כתיבות-סרק בסנכרון (שורש הכובד)**
`ORDERS.STATUSDATE` הוא ברזולוציית יום, אז ה-watermark נתקע על חצות והחלון כיסה יומיים בכל ריצה. `runAdoption` דחף את כל השורות ל-upsert בלי לבדוק אם משהו השתנה → `updated_at` קפץ → אירוע realtime לכל שורה → כל לקוח פתוח שאב מחדש 6,005 הזמנות + 5,552 קריאות, כל 20 דקות.
- `api/priority-sync.ts`: השוואה מול הערכים הקיימים לפני כתיבה (`sameValue`), גם ב-`runAdoption` וגם ב-`priority_customers` (`synced_at` מוחרג). נוסף שדה `unchanged` לתשובה.
- 🔴 **שתי מלכודות שהכשילו את הניסיון הראשון:** Postgres שומר `jsonb` עם סדר מפתחות משלו (לפי אורך המפתח), אז `JSON.stringify` תמיד יצא שונה → נוסף `canonical()` שממיין מפתחות רקורסיבית. ועמודות תאריך אף פעם לא חוזרות זהות (פריוריטי שולח עם offset, `date` מחזיר `YYYY-MM-DD`) → השוואה לפי הרגע בזמן, או לפי היום כשצד אחד date-only.
- **אומת בפרודקשן:** 141 שורות נכתבו בריצה הקודמת מול 1 בריצה שאחרי. ריצה שנייה = 0 כתיבות.

**ב. הקלה על הקליינט**
- `useRealtimeSync`: צבירת אירועים בחלון 1.5 שניות במקום `invalidate` לכל אירוע.
- `App.tsx` + 6 hooks: `refetchOnWindowFocus: false`, `staleTime` 60 שניות. הרעננות מגיעה מה-realtime.
- `OrdersTable` / `ServiceCallsTable`: רינדור 100 שורות עם `LoadMore`. שתי הטבלאות רנדרו את כל השורות **וגם** את כרטיסי המובייל במקביל (מוסתרים ב-CSS אך קיימים ב-DOM) — בערך 80,000 אלמנטים, ומחדש בכל הקלדה.
- `DATA_WINDOW_DAYS = 180` ב-`constants.ts` כשומר-גדילה (לא חוסך היום, כל הנתונים צעירים ממנו).

**ג. שורש הזבל: הסטטוס של פריוריטי לא נמשך**
האיסופים היו בריאים כי `STATDES` היה ממופה. בהזמנות `ORDSTATUSDES` נמשך ונזרק, ובקריאות `CALLSTATUSCODE` **כלל לא היה ב-$select** → 96% מההזמנות "ממתין לתאום" ו-96% מהקריאות "קריאה חדשה", לנצח.
- **ערכי הסטטוס ברשעל:** הזמנות = טיוטא · מאושרת לבצוע · בוצעה · שולמה · מבוטלת. קריאות = לביצוע · שובצה · בוצעה · סופית · מבוטלת · להמשך טיפול.
- **מיפוי (סופיים בלבד):** בוצעה/שולמה → `סופק` · מבוטלת → `בוטל` · בוצעה/סופית → `בוצע`. מה שפתוח בפריוריטי **לא נוגע** בסטטוס שלנו, כדי לא לדרוס את הסדרן. **"שובצה" לא ממופה בכוונה** — המשמעות ברשעל טרם אומתה מול הלקוח.
- טיוטא לא מוצגת לסדרן (`useZonedOrders` / `useZonedServiceCalls`).
- מיגרציה `20260805_priority_status_and_archive.sql`: `priority_status` + `archived_at`/`archived_reason` + אינדקסים חלקיים.

**ד. שאריות ה-webhook הישן**
ה-webhook של Make ייצר רשומות בלי `priority_*_id` (קריאות ריקות לגמרי) ו**הפסיק ב-04/08/2026** — הגוש סופי ולא גדל. אורכבו רק רשומות שאף אחד לא נגע בהן ואין להן שיבוץ ביומן.

**תוצאות ההרצה (אחרי דוח יבש שאושר, עם גיבוי מלא ב-`cleanup_snapshot_20260805`):**
| | לפני | אחרי |
|---|---|---|
| הזמנות במסד / גלויות | 6,010 | 1,722 |
| הזמנות ברשימת הסדרן | 5,766 | 464 |
| קריאות במסד / גלויות | 5,553 | 1,757 |
| קריאות ברשימת הסדרן | 5,356 | 540 |

נסגרו 1,276 הזמנות ו-1,330 קריאות · אורכבו 4,288 הזמנות ו-3,796 קריאות.

**עבודות חדשות ב-`rashal-sync`** (כולן דרך `{"job":"..."}`):
`probe` (גילוי סכימה — הדרך היחידה, ה-credentials הם Edge Function secrets) · `report-cleanup` (דוח יבש, `entity: orders|service_calls`) · `apply-cleanup` (הרצה) · `report-stuck-stops` (רשומות שנסגרו אך יש להן עצירה פתוחה).

**פתוח:** 77 רשומות נסגרו אך נשארה להן עצירה פתוחה ביומן (כולן בתאריכי עבר, אין נסיעה עתידית). לא נגענו ביומן בכוונה. רשימה: `~/Idan-HQ/meetings/rashal/2026-08-05-רשומות-סגורות-עם-שיבוץ-פתוח.html`. וגם: לברר מול רשעל מה המשמעות של "שובצה".


### 27/07/2026 — מסך סדרן מאוחד ⭐⭐⭐⭐⭐

**שינוי UX מרכזי:** שלושת מסכי הסדרן (משלוחים / קריאות שירות / איסופים) אוחדו למסך אחד.

- **`src/pages/DispatchPage.tsx` חדש (ראוט `/dispatch`)** — טאבים לסוג פעילות למעלה (הטאב נשמר ב-`?tab=deliveries|service|pickups`), רשימת הממתינים של הסוג הנבחר באמצע, ויומן `DeliveryCalendar` **יחיד וקבוע** למטה. היומן לא נהרס במעבר טאב — השבוע הנבחר, קיבוצים מכווצים וכל ה-state נשמרים. DndContext אחד + handlers גנריים לפי `kind` (delivery/service/pickup) החליפו את שלושת העותקים הכפולים.
- **הפניות:** `/routes`, `/service-calls`, `/pickups` → `Navigate` ל-`/dispatch?tab=...`. ב-`AppHeader` קישור אחד "מסך סדרן" במקום שלושה; כפתור הרענון מרענן עכשיו גם `pickups` + `calendarStops`.
- **יישור יכולות איסופים:** בדיקת כפילויות לפני שיבוץ (`findActiveDuplicateStops` + דיאלוג), יצירת משימה מהיומן, והעברת עצירה ליום אחר — עכשיו גם לאיסופים (היו רק במשלוחים/שירות).
- **צ'יפי סינון יומן:** מעל היומן שורת צ'יפים (משלוחים/שירות/איסופים/משימות) להצגה/הסתרה של סוגים, נשמר ב-localStorage (`rashal:calendarTypeFilter`). `buildCalendarDeliveries` מקבל filter.
- **קומפקטיות (בקשת עידן):** hook חדש `src/hooks/usePersistedCollapse.ts` — כיווץ שנשמר ב-localStorage. דגלי כיווץ בכל אזור: שורת סטטוס, "חזרו מהקו", סינון לפי אזור, רשימות הממתינים. תקרות גובה: רשימות 380px (קיבוץ 440px), "חזרו מהקו" 240px עם גלילה פנימית — היומן תמיד במרחק גלילה קצר.
- **Dead code:** DeliveriesPage / ServiceCallsPage / PickupsPage הישנים כבר לא מנותבים — למחיקה אחרי תקופת התייצבות.
- מסמך ההצעה שאושר: `~/Idan-HQ/meetings/rashal/2026-07-27-מסך-סדרן-מאוחד.html`
...
הצוות האמיתי הוזן (היה enum בן 2 ערכים בלבד):
- **נהגים (4):** דוד, רודי, מוחמד, מוהנד · **טכנאים (3):** אולג, ישראל, אבי · **דוד = נהג וגם טכנאי** (בשני הבוררים).
- **DB migration (אטומי):** `ALTER TYPE driver_name RENAME VALUE 'רודי דויד'→'רודי'` ו-`'נהג חיצוני מועלם'→'מוחמד'` (כל השיבוצים+הפרופיל עברו), `ADD VALUE` ל-דוד/מוהנד/אולג/ישראל/אבי. ערכים סופיים: רודי, מוחמד, דוד, מוהנד, אולג, ישראל, אבי.
- **`src/types/route.ts`:** `DriverName` (4), `TechnicianName` (3+דוד), `AssigneeName=DriverName|TechnicianName`, `DRIVERS`, `TECHNICIANS`, `TECHNICIAN_ONLY` (לניתוב משימות), `ASSIGNEE_PHONES` (טלפונים — שמירה בלבד, ללא תצוגה).
- **`src/types/delivery.ts`:** `DRIVER_CONFIG` ל-7 הערכים (צבע ייחודי לכל אחד) + `assigneeStyle()` עם fallback בטוח; `CalendarStop.driver`/`CalendarDelivery.driver` → `AssigneeName`. `calendar-stop.ts` בהתאם.
- **בורר גנרי** `DriverSelector` (`assignees?`+`title?`): משלוחים=DRIVERS, שירות=TECHNICIANS ("בחר טכנאי"). חל גם על שיבוץ-מחדש ו-`TaskDialog`.
- **ניתוב משימות:** task של טכנאי-בלבד (אולג/ישראל/אבי) → מסך שירות; task של נהג/דוד → מסך משלוחים (`TECHNICIAN_ONLY`).
- `DayMapDialog`: `DRIVER_ORDER` דינמי מכל המשובצים + טאבים רק למי שיש לו עצירות; קונפיג בטוח דרך `assigneeStyle`.
- טכנאים = רשימה קבועה ללא פרופיל/התחברות → אין דשבורד-טכנאי (כמתוכנן).

#### B) טוסטים מעוצבים (רוחבית)
- `src/lib/scheduleToast.tsx` — `showScheduleToast()`: הודעת שיבוץ בולטת (אייקון ✓ ירוק, ספירה, "לנהג/לטכנאי {שם}", יום) בשני המסכים.
- **`<Toaster>` גלובלי (`App.tsx`):** `richColors`+`closeButton`+`expand`+`dir=rtl`+`classNames` (rounded-2xl, צל, כותרת מודגשת, אייקון מוגדל) → כל הטוסטים במערכת משודרגים אוטומטית.
...
**עודכן לאחרונה:** 12 באוגוסט 2026: ארבע רשימות הממתינים אוחדו לרכיב `UnscheduledPanel` אחד, ואיסופים ולקוחות חדשים נראים ומתנהגים כמו הזמנות וקריאות שירות
